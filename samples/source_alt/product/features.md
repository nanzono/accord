...
`capability_categories` が決める。表の 1 列目が識別子で、パッケージはこの識別子で機能を束ねる
（accord の欄の名前は「ID」で、読み替えは設定が持つ）。裏づけの節には、`career_master.md` の
職歴の枠か受託案件の識別子を書き、複数あるときは半角の `/` で区切る。
...
| plan-from-decisions | 決まったことを段取りに落とす | 決まったことを工程と期日に分け、遅れが出た時点で分かる形にする | nagisa-publishing |
